...
def generate_verification_report(document_type: str, extracted_data: dict, file_path: str) -> dict:
    report = {
        ...
        "confidence_score": 95.0, # Base confidence increased because LLM is highly accurate
    ...
    # --- 1. Define strict enterprise-level fields based on LLM Schema ---
    required_fields = []
    if document_type == "CNIC":
        required_fields = ["name", "father_name", "identity_number", "date_of_birth", "date_of_issue", "date_of_expiry"]
        ...
        # Updated to match the exact keys generated by Gemini
        required_fields = ["name", "emails", "phone_numbers", "linkedin_url", "github_url", "university", "skills"]
        
    elif document_type == "Transcript":
        required_fields = ["name", "university", "degree", "cgpa", "graduation_year"]

    # --- 2. Dynamic Field Checking ---
    present_fields = 0
    for field in required_fields:
        value = extracted_data.get(field)
        # Check if the value exists and is not empty (handles strings, lists, and numbers)
        if value and (isinstance(value, (int, float)) or len(str(value)) > 0):
            present_fields += 1
        else:
